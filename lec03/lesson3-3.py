...
# 入力例:
#   line: ['3', '.', '0', '+', '8'], index: 0 
#       → token: {'type': 'NUMBER', 'number': 3.0}, index: 3}
...
# tokensから"()"をなくした式を返す関数
# 入力: |tokens| = 式をトークン化したもの
# 出力: |new_tokens| = 入力式に含まれる"()"内を計算することで、"()"を取り除いた式のトークン
def bracket(tokens):
    index = 0
    new_tokens = []

    while index < len(tokens):
        if tokens[index]['type'] == 'LEFT': #  "("があったら
            index_left = 1
            index_new = index + 1
            bracket_tokens = []

            while index_new < len(tokens): # 対応する")"を探し、()内の式をbracket_tokensに格納
                if tokens[index_new]['type'] == 'LEFT':
                    index_left += 1
                elif tokens[index_new]['type'] == 'RIGHT':
                    index_left -= 1

                if index_left == 0:
                    break

                bracket_tokens.append(tokens[index_new])
                index_new += 1
            
            bracket_ans = evaluate(bracket_tokens) # ()内を計算
            new_tokens.append({'type': 'NUMBER', 'number': bracket_ans})
            index = index_new + 1
        else:
            new_tokens.append(tokens[index])
            index += 1
...
# |ans| = 式の解
def evaluate(tokens):
    remove_bracket = bracket(tokens)
    cal_mul_div = evaluate_mul_div(remove_bracket)
